package com.xupu.locationmap.common.tdt;
        ...
        if (layerInfo != null) {
            if (tileImage == null) {
                ...
                String path = layerInfo.getUrl()
                        ...
                        + layerInfo.getLayerName() + "&STYLE=default&FORMAT=tiles&TILEMATRIXSET="
                        + layerInfo.getTileMatrixSet() + "&TILECOL=" + col
                        + "&TILEROW=" + row + "&TILEMATRIX=" + (level) + "&tk=797aad3cd34133bc98090ea7963b6b1c";


               /* OkHttpClient client = new OkHttpClient.Builder().readTimeout(5, TimeUnit.SECONDS).build();
                Request request = new Request.Builder().addHeader("referer", "ArcGISRuntime-Android/100.3").url(path)
                        .get().build();
